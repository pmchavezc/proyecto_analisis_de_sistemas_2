...
export default function RequestsList() {
  ...
  const filteredRequests = requests.filter((request) => {
    if (estadoFilter && request.status !== estadoFilter) return false;
    if (prioridadFilter && request.priority !== prioridadFilter) return false;
    if (fechaFilter && request.date !== fechaFilter) return false;
    return true;
  });

  // Ordenar por ID descendente (más recientes primero)
  const sortedRequests = [...filteredRequests].sort((a, b) => a.id - b.id);
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-gray-50">
        <Header title="Gestión de Mantenimiento Urbano" showSearch={false} />
            ...
            {!loading && !error && filteredRequests.length > 0 && (
              <>
                <RequestsTable
                  ...
                  onSync={refetch} // ✅ Aquí agregás la prop
                />

                <div className="mt-4 text-sm text-gray-600">
                  Mostrando 1-{filteredRequests.length} de {requests.length}{" "}
                  resultados
                  {filteredRequests.length !== requests.length &&
                    ` (filtrado de ${requests.length})`}
                </div>
              </>
            )}
